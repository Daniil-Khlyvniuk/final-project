		...
		<Box className={classes.container}>
			...
			<Box className={classes.desc}>
				<Typography
					...
					{!card && <svg onClick={() => handleShoppingBag.removeAll(item?._id)} className={classes.cross} width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
						<path
				...
				{!card && (<>
					<Box className={classes.specificationsContainer}>
						<Box className={classes.specifications}>
							<Typography fontSize={18}>{Storquantity <= 0 ? 'Out of stock': `in stock: ${Storquantity}`}</Typography>
						</Box>
