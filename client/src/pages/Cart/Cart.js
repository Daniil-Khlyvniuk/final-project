	...
	return (
		<div className={classes.container}>
			...
			<div className={classes.desc}>
				...
				<Typography fontSize={14} sx={{mb: '14px', mt: '85px'}} variant={'p'}>This is the luxury bedding set with
					absolutely everything
					in it, at a price that won't keep you up at night.</Typography>
				<Typography fontSize={18} sx={{mb: '14px', mt: '85px'}} variant={'p'}>{price}</Typography>
				<svg onClick={remove} className={classes.cross} width="20" height="20" viewBox="0 0 20 20" fill="none"
...
const Cart = () => {
	...
	return (
		<Container maxWidth="lg">
			...
			<div className={classes.cards}>{shoppingBag?.map((item, key) => <Card key={key}
																																						_id={item._id}
																																						title={item.title}
																																						image={item.image}
